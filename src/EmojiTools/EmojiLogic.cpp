...
char8_t EmojiBuilder::getEmojiChar8_tCharByCodePoint(char32_t *emojiCodePoints,
                                                     size_t length) {
  char8_t buffer[32];
  char8_t *end = encodeUtf8Sequence(emojiCodePoints, length, buffer);
  *end = '\0'; // Null-terminating the string
  ...
  if (emojiBuilder.m_isPopulated) {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap) {
      if (std::find(vecGroups.begin(), vecGroups.end(),
                    epm.second.m_emojiGroup) == vecGroups.end()) {
  ...
  if (emojiBuilder.m_isPopulated) {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap) {
      if (std::find(vecSubGroups.begin(), vecSubGroups.end(),
                    epm.second.m_emojiSubGroup) == vecSubGroups.end()) {
  ...
  if (emojiBuilder.m_isPopulated) {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap) {
      if (std::find(subgroups.begin(), subgroups.end(),
                    epm.second.m_emojiSubGroup) == subgroups.end()) {
